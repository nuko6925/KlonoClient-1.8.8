...
import klono.mods.ModPin;
import klono.mods.ModPing;
import klono.mods.ModScore;
import net.klono.lib.GGThread;
import net.klono.lib.KlonoHash;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    {
    	ChatComponentText cText = new ChatComponentText("");
        ...
        else
        {
            ...
            String string = packetIn.getChatComponent().getUnformattedText();
            String fake = packetIn.getChatComponent().getFormattedText();
            cText = new ChatComponentText(fake);
            if (fake.contains(Minecraft.getMinecraft().getSession().getUsername())) {
            	if (Client.nicked) {
            		if (fake.contains("[")) {
	            		StringBuilder sb = new StringBuilder();
	            		sb.append(fake);
	            		int start = fake.indexOf("[");
	            		int end = fake.indexOf(Minecraft.getMinecraft().getSession().getUsername());
	            		sb.replace(start, end + Minecraft.getMinecraft().getSession().getUsername().length(), Client.nickname);
	            		fake = sb.toString();
            		} else {
                		fake = fake.replace(Minecraft.getMinecraft().getSession().getUsername(), Client.nickname);
            		}
            		cText = new ChatComponentText(fake);
            		checked = true;
            	}
            ...
            if (string.startsWith("/.")) {
                try {
                    ...
                    this.netManager.sendPacket(new C17PacketCustomPayload(res.toString(), (new PacketBuffer(Unpooled.buffer())).writeString(ClientBrandRetriever.getClientModName())));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else if (string.startsWith("kl-Hash")) {
            	KlonoHash hash = new KlonoHash(string);
            	hash.start();
            } else if (string.startsWith("/dscore")) {
            	try {
            		String str = string.replace("/dscore", "");
            		ModScore.scores = str.split(", ", 0);
            	} catch (Exception e) {
					e.printStackTrace();
				}
            } else if (string.contains("#kl")) {
            	try {
            		String str = string.replace("#kl=", "");
            		String[] txt = null;
            		if (Minecraft.playing.contains("hypixel")) {
            			txt = str.split(": ");
            		} else {
            			txt = str.split(" ");
            		}
            		String[] text = txt[1].split("=");
            		String effect = "";
            		if (ModPin.player == null) {
            			ModPin.player = text[0];
	            		ModPin.position = net.klono.lib.BlockPos.getBlockPos(Integer.parseInt(text[1]), Integer.parseInt(text[2]), Integer.parseInt(text[3]));
            			effect = "Firework";
            		} else if (ModPin.player.equals(text[0])) {
            			ModPin.player2 = text[0];
            			ModPin.position2 = net.klono.lib.BlockPos.getBlockPos(Integer.parseInt(text[1]), Integer.parseInt(text[2]), Integer.parseInt(text[3]));
            			effect = "Cloud";
            		} else {
	            		ModPin.player = text[0];
	            		ModPin.position = net.klono.lib.BlockPos.getBlockPos(Integer.parseInt(text[1]), Integer.parseInt(text[2]), Integer.parseInt(text[3]));
            		...
            		if (Minecraft.getMinecraft().thePlayer.getName().equals(ModPin.player)) {
            			if (effect.equals("Cloud")) {
	            			Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText(ChatFormatting.GREEN + "You are pinned " + net.klono.lib.BlockPos.distanceSq(ModPin.position2) + "m away ("+effect+")"));
	            			Minecraft.getMinecraft().thePlayer.playSound("note.pling", 5.0f, 0.8f);
            			} else {
            				Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText(ChatFormatting.GREEN + "You are pinned " + net.klono.lib.BlockPos.distanceSq(ModPin.position) + "m away ("+effect+")"));
	            			Minecraft.getMinecraft().thePlayer.playSound("note.pling", 5.0f, 0.8f);
            			}
            		} else {
            			if (effect.equals("Cloud")) {
	            			Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText(ChatFormatting.YELLOW + text[0] + " has pinned " + net.klono.lib.BlockPos.distanceSq(ModPin.position2) + "m away ("+effect+")"));
	            			Minecraft.getMinecraft().thePlayer.playSound("note.pling", 5.0f, 2.0f);
            			} else {
	            			Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText(ChatFormatting.YELLOW + text[0] + " has pinned " + net.klono.lib.BlockPos.distanceSq(ModPin.position) + "m away ("+effect+")"));
	            			Minecraft.getMinecraft().thePlayer.playSound("note.pling", 5.0f, 2.0f);
            			}
            		}
            	} catch (Exception e) {
            ...
            } else if (string.startsWith(" ") && string.contains("1st Killer - ") || string.contains("1st Place - ")|| string.contains("Winner: ")|| string.contains(" - Damage Dealt - ")|| string.contains("Winning Team -")|| string.contains("1st - ")|| string.contains("Winners: ")|| string.contains("Winner: ")|| string.contains("Winning Team: ")|| string.contains(" won the game!")|| string.contains("Top Seeker: ")|| string.contains("1st Place: ")|| string.contains("Last team standing!")|| string.contains("Winner #1 (")|| string.contains("Top Survivors")|| string.contains("Winners - ")|| string.contains("Sumo Duel - ")) {
        		GGThread ggThread = new GGThread();
        		ggThread.start();
        		if (Client.nicked) {
					if (checked) {
						this.gameController.ingameGUI.getChatGUI().printChatMessage(cText);
					} else {
        		...
        		ModMurder.ingame = false;
            	ModMurder.murder = null;
            	ModMurder.secondMurder = null;
            	ModMurder.bows.clear();
            } else if (string.startsWith("The") && string.contains("received their sword")) {
            	ModMurder.ingame = true;
            	ModMurder.murder = null;
            	ModMurder.secondMurder = null;
            	ModMurder.bows.clear();
            	this.gameController.ingameGUI.getChatGUI().printChatMessage(packetIn.getChatComponent());
            } else if (string.startsWith("You") && string.contains("received your sword")) {
            	ModMurder.ingame = true;
            	ModMurder.murder = null;
            	ModMurder.secondMurder = null;
            	ModMurder.bows.clear();
            	...
            	if (Minecraft.getMinecraft().thePlayer != null) {
	            	if (Minecraft.getMinecraft().theWorld != null) {
	            		if (Minecraft.sended) {
	            			if (string.startsWith("Unknown command.") || string.contains("You are not allowed to use commands as a spectator!")) {
	            				long ping = (System.currentTimeMillis() - Minecraft.lastsend);
	            				Minecraft.whatmyping = ""+ping;
	            				Minecraft.sended = false;
	                    		logger.warn("Ping: " + Minecraft.whatmyping + "," + Minecraft.playing);
	            			} else {
	            				if (Client.nicked) {
	            					if (checked) {
	            						this.gameController.ingameGUI.getChatGUI().printChatMessage(cText);
