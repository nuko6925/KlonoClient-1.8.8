...
import net.klono.lib.LevelHead;
    ...
    public void renderName(T entity, double x, double y, double z)
    {
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, new Object[] {entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)}))
        {
            if (this.canRenderName(entity))
            {
                ...
                {
                	String s = "";
                	String klono_mark = SessionAPI.klono_mark;
                	if (SessionAPI.Klono_Users.contains(entity.getUniqueID().toString())) {
						//Level Head
			            float heart = LevelHead.getLevel(entity.getName());
			            if (heart != 0) {
			            	if (entity.getDisplayName().getFormattedText().contains(Minecraft.getMinecraft().getSession().getUsername())) {
			                	if (Client.nicked) {
			                		s = klono_mark + Client.nickname + "˜r : " + heart + "˜c\u2764˜r";
			                	} else {
			                		s = klono_mark + entity.getDisplayName().getFormattedText() + "˜r : " + heart + "˜c\u2764˜r";
			                	}
			                } else {
			                	s = klono_mark + entity.getDisplayName().getFormattedText() + "˜r : " + heart + "˜c\u2764˜r";
			                }
			                ...
			                if (entity.getDisplayName().getFormattedText().contains(Minecraft.getMinecraft().getSession().getUsername())) {
			                	if (Client.nicked) {
			                		s = klono_mark + Client.nickname;
			                	} else {
			                		s = klono_mark + entity.getDisplayName().getFormattedText();
			                	}
			                } else {
			                	s = klono_mark + entity.getDisplayName().getFormattedText();
			                }
	                	...
	                	if (heart != 0) {
	                		if (entity.getDisplayName().getFormattedText().contains(Minecraft.getMinecraft().getSession().getUsername())) {
	                			if (Client.nicked) {
	                				s = Client.nickname + "˜r : " + heart + "˜c\u2764˜r";
	                			} else {
	                				s = entity.getDisplayName().getFormattedText() + "˜r : " + heart + "˜c\u2764˜r";
	                			}
	                		} else {
	                			s = entity.getDisplayName().getFormattedText() + "˜r : " + heart + "˜c\u2764˜r";
	                		}
                    ...
                    if (entity.isSneaking())
                    {
                        ...
                        if (Minecraft.getMinecraft().gameSettings.tOGGLE_NAMETAG.getKeyCode() == Keyboard.KEY_HOME) {
...
//        return Minecraft.isGuiEnabled() && entity != this.renderManager.livingPlayer && !entity.isInvisibleToPlayer(entityplayersp) && entity.riddenByEntity == null;
        if (Minecraft.getMinecraft().gameSettings.CHEAT.getKeyCode() == Keyboard.KEY_DELETE) {
        	return Minecraft.isGuiEnabled() && entity.riddenByEntity == null;
        }
