        ...
        (new Thread("Server Connector #" + CONNECTION_ID.incrementAndGet())
        {
            public void run()
            {
                ...
                try
                {
                    ...
                    if (ModPing.DRPC) {
                    	Client.getInstance().getDiscordRP().update("", "IP:" + ip + (port != 25565 ? ":" + port: ""));
                    } else {
                    	Client.getInstance().getDiscordRP().update("", "IP: Hide");
                    }
                    Minecraft.playing = ""+ip;
                    CheckMultiplayer.raw_ip = inetaddress.toString() + ":" + port;
                }
