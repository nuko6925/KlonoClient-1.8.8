...
public class GuiPlayerTabOverlay extends Gui
{
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, ScoreObjective scoreObjectiveIn)
    {
        ...
        for (NetworkPlayerInfo networkplayerinfo : list)
        {
            ...
            if (mc.gameSettings.TOGGLE_PINGTAB.getKeyCode() == Keyboard.KEY_HOME) {
            	String str = "";
            	if (networkplayerinfo.getResponseTime() == 0) {
            		str = "...";
            	} else {
            		str = ""+networkplayerinfo.getResponseTime();
            	}
            	k = this.mc.fontRendererObj.getStringWidth(getPlayerName(networkplayerinfo) + str);
            }
            i = Math.max(i, k);
        ...
        for (int k4 = 0; k4 < l3; ++k4)
        {
            ...
            if (k4 < list.size())
            {
                ...
                {
                	if (s1.contains(mc.getSession().getUsername())) {
                		if (Client.nicked) {
                			s1 = Client.nickname;
                		}
                	}
                	if (ModMurder.bows.size() >= 1) {
                		for (String bow : ModMurder.bows) {
                			if (s1.contains(bow)) {
                				s1 = ChatFormatting.YELLOW+"[B] "+s1;
                			}
	                	}
                	}
                	if (ModMurder.murder != null) {
	                	if (s1.contains(ModMurder.murder)) {
	                		s1 = ChatFormatting.RED+"[M] "+s1;
	                	}
                	}
                	if (ModMurder.secondMurder != null) {
	                	if (s1.contains(ModMurder.secondMurder)) {
	                		s1 = ChatFormatting.RED+"[M] "+s1;
	                	}
                	}
                	List<String> temp_Users = SessionAPI.Klono_Users;
                	if (temp_Users.size() != 0) {
                		try {
    	                	for (String string : temp_Users) {
    							if (string.contains(gameprofile.getId().toString())) {
    								s1 = SessionAPI.klono_mark + s1;
    							}
    						}
    ...
    protected void drawPing(int p_175245_1_, int p_175245_2_, int p_175245_3_, NetworkPlayerInfo networkPlayerInfoIn)
    {
        ...
        if (mc.gameSettings.TOGGLE_PINGTAB.getKeyCode() == Keyboard.KEY_HOME) {
        	...
        	if (networkPlayerInfoIn.getResponseTime() == 0) {
        		...
        		str = ""+networkPlayerInfoIn.getResponseTime();
        	}
        	this.mc.fontRendererObj.drawStringWithShadow(ChatFormatting.GREEN + str, p_175245_2_ + p_175245_1_ - 11 + 11 - this.mc.fontRendererObj.getStringWidth(""+networkPlayerInfoIn.getResponseTime()), p_175245_3_, -1);
        } else {
