...
import klono.Client;
import net.klono.lib.TextColorFormat;
import net.klono.lib.TransApi;
...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback
{
    ...
    public void sendChatMessage(String msg)
    {
    	if (msg.startsWith(".nick")) {
    		String args = msg.replace(".nick", "");
    		if (args != null && !args.equals("")) {
    			String name = TextColorFormat.getColored(args);
    			mc.thePlayer.addChatMessage(new ChatComponentText(ChatFormatting.GREEN + "You are nicked to " + ChatFormatting.RESET + name));
    			Client.nickname = name;
    			Client.nicked = true;
    		} else {
    			mc.thePlayer.addChatMessage(new ChatComponentText(ChatFormatting.GREEN + "You are unnicked to " + ChatFormatting.RESET + mc.getSession().getUsername()));
    			Client.nickname = mc.getSession().getUsername();
    			Client.nicked = false;
    		}
    	} else {
    ...
    public void handleMouseInput() throws IOException
    {
        ...
        {
            if (GameSettings.touchscreen && this.touchValue++ > 0)
            {
                return;
            }
